...
2. **Parity for Card / EFTPOS / Cash** — either dedicated summary views (period totals + trip list, analogous to TM Trip History / Monthly Invoicing / ACC Billing) **or** one shared “Payments” hub with a payment-type filter that makes each type first-class. Include fixing/removing the broken dashboard **Card Payments → CardCommission.aspx** link.

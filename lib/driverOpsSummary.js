 ...
 * TM: cash/EFTPOS/Account remainder stays $0-owed (tracked only); Card remainder
 * enters the platform Card Mark Paid stream on passengerPays only (commission-adjusted).
 * Council subsidy still enters pay.tm.owed. PaymentType===TM never owes full fare —
 * subsidy only. Hoist is a separate owed line summed with subsidy for the TM stream.
 */
export function jobPaymentLines(job, cardSettings = {}) {
  ...
  if (tmJob) {
    // Gross = passenger portion only — never full meter (council subsidy is not cash/card held).
    ...
    if (TRACKED_NOT_PLATFORM_OWED.includes(main.bucket)) {
      ...
      // Platform Card Mark Paid — commission on passenger remainder only.
      const cardLine = companyOwesDriver(paxGross, pm, cardSettings);
      lines.push({
        kind: 'main',
        bucket: 'card',
        gross: paxGross,
        owed: cardLine.owed,
        commission: cardLine.commission,
      });
    }
    // PaymentType TM / other on a TM job: do not put full fare into tm.owed.
    if (parts.subsidy > 0) {
      lines.push({
        kind: 'tm_subsidy',
        bucket: 'tm',

...
/**
 * BookaWaka Mark Paid settles these buckets (platform-mediated money).
 * Future channels (food/freight/rentals/towing/pax-app/web) should plug into this list
 * or a parallel company-ledger list — not ad-hoc owedTotal sums.
 */
...
export function buildDriverSummaryRow(opts) {
  ...
  // BookaWaka Mark Paid = PLATFORM_OWED_BUCKETS only (card + TM + hoist).
  // Account/ACC/cash/eftpos stay visible via pay.*.gross/count — Track B settles Account separately.
  const owedTotal =
    pay.card.owed + pay.tm.owed + pay.hoist.owed + pay.other.owed;

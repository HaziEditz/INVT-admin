...
/**
 * Meter-side TM subsidy (council) + display percentages.
 ...
 * Council/passenger %: stored fields, else subsidy÷(fare−hoist) and passenger÷(fare−hoist).
 */
...
export function jobPaymentLines(job, cardSettings = {}) {
  ...
  if (tmJob) {
    ...
    const { subsidy } = tmSubsidyParts(job);
    if (subsidy > 0) {
      lines.push({
        kind: 'tm_subsidy',
        bucket: 'tm',
...
export function buildDriverSummaryRow(opts) {
  ...
  if (paxPctSamples.length) {
    tmDetail.passengerPct =
      Math.round((paxPctSamples.reduce((a, b) => a + b, 0) / paxPctSamples.length) * 10) / 10;
  } else if (tmDetail.councilPct != null) {

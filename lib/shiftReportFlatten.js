 ...
 * Prefer real sessionStartedAt when present. Legacy progressive End Shift writes
 * reuse shiftStartAt — collapse those so we do not invent false offline gaps.
 */
function buildDayTimeline(sessions) {
  ...
  collapsed.forEach(function (s) {
    ...
    usable.push({
      ...
      hasSessionStart: !!(s._hasSessionStart || s.sessionTs || s._sessionTs),
    });
  });
  ...
  // Gaps are only reliable when every stint has a real session login timestamp.
  var gapsReliable = usable.every(function (u) {
    return u.hasSessionStart;
  });
  ...
  return {
    ...
    offlineKnown: gapsReliable && multi,
    onlinePct: onlinePct,

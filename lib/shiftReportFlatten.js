 ...
 * Sessions need start+end; overlapping gaps are skipped (0). Offline summary =
 * span − online when 2+ sessions; single session → offline null (display "—").
 */
function buildDayTimeline(sessions) {
  ...
  var onlinePct =
    spanMin > 0 ? Math.min(100, Math.round((100 * onlineMin) / spanMin)) : null;
